    ...
    public class AD_CreateOrderController : CustomController
    {
        ...
        public ActionResult PartialDetail(string id)
        {
            if (userAsset.ContainsKey("View") && userAsset["View"])
            {
                ...
                dict["SONumber"] = id;
                dict["MerchantByUser"] = dbConn.Select<Auth_User_Merchant>(s => s.UserID == currentUser.UserID).FirstOrDefault();
                if (string.IsNullOrEmpty(id))
                {
                    return PartialView("_AD_CreateOrderIndex", dict);
                }
                else
                {
                    return PartialView("_AD_CreateOrder", dict);
                }
            }
            else
            {
        ...
        //    }
        //    var data = new SODetail().GetPage(request, whereCondition, SONumber);
        //    return Json(data);
        //}
                    ...
                    foreach (var item in list)
                    {
                        if (item.Qty > 0)
                        {
                            ...
                            else
                            {
                                ...
                                data.UnitName = dbConn.Select<DC_AD_Unit>(s => s.UnitID == item.Unit).FirstOrDefault().UnitName;
