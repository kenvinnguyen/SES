...
$(document).ready(function () {
    ...
    //active menu
    resetMenu();
    activeMenu(1, 2, 1, '2_1', 'menu_RuleCheck');

    document.title = "Danh sách rule";
                ...
                success: function (data) {
                    if (data.success) {
                        if (keyAction == 0) {   // Create
                            $("#formPopup").find('fieldset:eq(0) section:eq(0)').empty().append('<div class="divlabel"><label class="label" style="float: right;">Mã Rules: </label></div><div class="divfile"><label style="float: right;text-align: left;width: 240px;height: 19px;padding-top: 2px;"><strong style="color:red;">' + data.RuleID + '</strong><input type="hidden" id="RuleID" name="RuleID" value="' + data.RuleID + '" /></label> <div style="clear:both"></div>');
...
function doSearch() {
    ...
    var text = $("#txtRuleID").val();
    if (text) {
        var filterOr = { logic: "or", filters: [] };
        filterOr.filters.push({ field: "RuleID", operator: "contains", value: text });
        filterOr.filters.push({ field: "RuleName", operator: "contains", value: text });
...
function onOpenPopup(key, obj) {
    ...
    if (key == 0) {     // Create
        ...
        var id = $(obj).data('id');
        $("#formPopup").find('fieldset:eq(0) section:eq(0)').empty().append('<div class="divlabel"><label class="label" style="float: right;">Mã Rule: </label></div><div class="divfile"><label style="float: right;text-align: left;width: 240px;height: 19px;padding-top: 2px;"><strong style="color:red;">' + id + '</strong><input type="hidden" id="RuleID" name="RuleID" value="' + id + '"/></label> <div style="clear:both"></div>');
        ...
        $.post(r + "/RuleCheck/GetRuleByID", { RuleID: id }, function (data) {
            showLoading();
...
function readHeaderInfo() {
    ...
    //contentTab = setContentTab(["RuleID", "PromotionName", "Status"], "30");

}

function onRefreshPopup() {
    $("#formPopup").find('fieldset:eq(0) section:eq(0)').empty().append('<div class="divlabel"><label class="label" style="float: right;">Mã Rule: </label></div><div class="divfile"><label class="input" style="float:right;width:240px;"><input type="text" id="RuleID" name="RuleID" class="input-xs" placeholder="Mã Rule" style="margin-right:85px" /></label><div style="clear:both"></div>');
    $("#RuleName").val('');
