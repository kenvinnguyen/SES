...
$(document).ready(function () {
    ...
    //active menu
    resetMenu();
    activeMenu(1, 2, 1, '2_1', 'menu_DeliveryDiscount');

    document.title = "Chương trình chiết khấu";
                ...
                success: function (data) {                   
                    if (data.success) {
                        if (keyAction == 0) {   // Create
                            $("#formPopupDe").find('fieldset:eq(0) section:eq(0)').empty().append('<div class="divlabel"><label class="label" style="float: right;">Mã CCKM: </label></div><div class="divfile"><label style="float: right;text-align: left;width: 240px;height: 19px;padding-top: 2px;"><strong style="color:red;">' + data.DiscountionID + '</strong><input type="hidden" id="DiscountionID" name="DiscountionID" value="' + data.DiscountionID + '" /></label> <div style="clear:both"></div>');
    ...
    $("#txtDiscountionID").keypress(function (e) {
        if (e.keyCode == 13) {
            doSearch();
        }
    });
...
function doSearch() {
    ...
    var text = $("#txtDiscountionID").val();
    if (text) {
        var filterOr = { logic: "or", filters: [] };
        filterOr.filters.push({ field: "DiscountionID", operator: "contains", value: text });
        filterOr.filters.push({ field: "DiscountionName", operator: "contains", value: text });
...
function onOpenPopup(key, obj) {
    ...
    if (key == 0) {     // Create
        keyAction = key;
        popup.title('Thêm');
        $("#DiscountionType").val('None')
        $("#DiscountionType").css('width', '240px');
        $("#DiscountionType").select2();
        onRefreshPopup();
        setTimeout(function () {
            $("#DiscountionID").focus();
        ...
        var id = $(obj).data('id');
        $("#formPopupDe").find('fieldset:eq(0) section:eq(0)').empty().append('<div class="divlabel"><label class="label" style="float: right;">Mã CTKM: </label></div><div class="divfile"><label style="float: right;text-align: left;width: 240px;height: 19px;padding-top: 2px;"><strong style="color:red;">' + id + '</strong><input type="hidden" id="DiscountionID" name="DiscountionID" value="' + id + '"/></label> <div style="clear:both"></div>');

        var currentRow = $(obj).closest("tr");
        var dataItem = $("#grid").data("kendoGrid").dataItem(currentRow);
        onBindDataToForm(dataItem);
        setTimeout(function () {
            $("#DiscountionName").focus();
        }, 500);        
        $.post(r + "/DeliveryDiscount/GetDiscountionCode", { DiscountionID: id }, function (data) {
            showLoading();
            ...
            if (data.success) {
                ...
                $("#DiscountionType").css('width', '240px');
                $("#DiscountionType").select2();
                $("#DiscountionType").val(value.DiscountionType);
...
function readHeaderInfo() {
    ...
    //contentTab = setContentTab(["DiscountionID", "PromotionName", "Status"], "30");

}

function onRefreshPopup() {
    $("#formPopupDe").find('fieldset:eq(0) section:eq(0)').empty().append('<div class="divlabel"><label class="label" style="float: right;">Mã CTKM: </label></div><div class="divfile"><label class="input" style="float:right;width:240px;"><input type="text" id="DiscountionID" name="DiscountionID" class="input-xs" placeholder="Mã CTCK" style="margin-right:85px" /></label><div style="clear:both"></div>');
    $("#DiscountionName").val('');
